<?php include('./connection.php'); ?>
	...
	<div class="container" style="margin-top:80px">
		...
		if(isset($_GET['id'])){
			...
			$select = mysqli_query($conn, "SELECT * FROM cinema WHERE id='$id'") or die(mysqli_error($conn));
		...
		if(isset($_POST['Submit'])){
			...
			$sql = mysqli_query($conn, "UPDATE cinema SET name='$name', img='$img' WHERE id='$id'") or die(mysqli_error($conn));

			if($sql){
				echo '<script>alert("Berhasil menyimpan data.");  document.location="cinema.php?page=tampil_data_cinema";</script>';
			}else{
...
<form action="cinema.php?page=edit_data_cinema&id=<?php echo $id; ?>" method="post">
	...
	<div class="item form-group">
				<div class="col-md-6 col-sm-6 offset-md-3">
					...
					<a href="cinema.php?page=tampil_data_cinema" class="btn btn-warning">Kembali</a>
				</div>
			</div>
		</form>
	</div>
